...
export function RecommendationActionDock() {
  ...
  // Map available recommendations according to API docs
  const recommendations = recommendationsData?.most_disliked_foods?.map(food => ({
      ...
      <Card className="bg-gradient-card border-card-border shadow-card">
        ...
        <CardContent className="space-y-4">
          {isLoading ? (
            ...
            <div className="space-y-3">
              {recommendations.slice(0, 3).map((swap, index) => (
